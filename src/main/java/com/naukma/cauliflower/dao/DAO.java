...
//                if (!connection.isClosed()) connection.close();
	...
	// Galya_Sh RI.6
	// Получить ServiceInstance по OrderId. По cable_id получить привязанный
	// порт и сделать его свободным. cable_id в ServiceInstance
	// сделать равным null. Сам кабель удалить из базы.
	// The system should allow deleting of Cables and Circuits.
	public void removeCableFromServiceInstanceAndFreePort(int serviceOrderId) {
		Connection connection = getConnection();
		...
		try {
			...
			try {
				...
				if (!preparedStatement.isClosed())
					preparedStatement.close();
				if (!preparedStatementUpdate.isClosed())
